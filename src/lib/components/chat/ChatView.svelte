<script lang="ts">
	...
	const store = new MessagesStore(roomId, userId, userName, userRole, dbMessages, roomName, ocrEnabled ?? false, ragEnabled ?? false);
...
{#if briefModalData}
	<BriefModal
		...
		agents={[...store.onlineUsers.entries()].filter(([, u]) => u.role === 'agent').map(([, u]) => ({ name: u.name }))}
		onAskAgent={(agentName: string) => { store.sendMessage(`@${agentName} please fill in the project brief using the brief_update tool`); }}
	/>
{/if}
